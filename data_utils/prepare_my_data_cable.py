...
def process_images():
    ...
    # 目标尺寸：先填充到1920x1920，再压缩到1024x1024
    target_width = 1024  # 最终尺寸
    target_height = 1024
    original_height = 1024

    # 需要填充的像素数（临时填充到1920x1920）
    temp_target_width = 1920
    temp_target_height = 1920
    padding_height = temp_target_height - original_height
        ...
        if os.path.exists(img_source_dir):
            ...
            for img_file in img_files:
                ...
                try:
                    ...
                    # 1. 先创建1920x1920黑色背景图像（临时）
                    temp_img = Image.new('RGB', (temp_target_width, temp_target_height), (0, 0, 0))

                    # 将原始图像粘贴到中间
                    temp_img.paste(img, (0, top_padding))

                    # 2. 压缩到1024x1024
                    final_img = temp_img.resize((target_width, target_height), Image.LANCZOS)
        ...
        if os.path.exists(mask_source_dir):
            ...
            for mask_file in mask_files:
                ...
                try:
                    ...
                    # 1. 先创建1920x1920黑色背景图像（临时）
                    temp_mask = Image.new('L', (temp_target_width, temp_target_height), 0)

                    # 将原始掩码粘贴到中间
                    temp_mask.paste(mask, (0, top_padding))

                    # 2. 压缩到1024x1024，使用最近邻插值（避免引入新像素值）
                    final_mask = temp_mask.resize((target_width, target_height), Image.NEAREST)
                    ...
                    # 验证像素值
                    final_array = np.array(final_mask)
                    unique_values = np.unique(final_array)
                    print(f"    掩码像素值分布: {unique_values}")

                    # 保存图像（使用PNG格式保持无损）
                    base_name = os.path.splitext(mask_file)[0]
                    target_path = os.path.join(target_seg_dir, f"{folder_name}_{base_name}.png")
                    final_mask.save(target_path, 'PNG')
    ...
    seg_count = len([f for f in os.listdir(target_jpeg_dir) if f.endswith('.png')])
